...
function scoresURLs(baseDate = new Date()) {
    ...
    const urls = [];
    urls.push(`https://www.goalserve.com/getfeed/${API_KEY}/golf/live?json=1`);

    // Get valid GOLF season dates for the past year
    const currentYear = today.getFullYear();
    const lastYear = currentYear - 1;
    
    // Helper function to determine GOLF season dates for a given year
    function getGOLFSeasonDates(year) {
    ...
    const seasons = [
        getGOLFSeasonDates(currentYear),
        getGOLFSeasonDates(lastYear)
    ];

    // Generate URLs for valid GOLF season dates within the past year
    const startDate = new Date(today);
    // Loop through the past 365 days but only include dates in FG seasons
    for (let i = 0; i < 365; i++) {
        startDate.setDate(startDate.getDate() - 1); // Move to previous day
        // Check if date falls within any of the defined GOLF seasons
        ...
        if (isInSeason) {
            ...
            urls.push(`https://www.goalserve.com/getfeed/${API_KEY}/golf/live?date=${dateString}&json=1`);
        }
    }
...
async function fetchGOLFData(sport, extractedData) {
    const sportLower = sport.toLowerCase();
    if (sportLower !== 'golf') {
        throw new Error('Only GOLF is valid for now');
    }

    const scoreUrls = scoresURLs();
    // const standingsUrl = `https://www.goalserve.com/getfeed/${API_KEY}/golf/pga_rankings?json=1`;
    let scheduleUrl = `https://www.goalserve.com/getfeed/${API_KEY}/golf/pga_schedule?json=1`;
