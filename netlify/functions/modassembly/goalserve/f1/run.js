...
function scoresURLs(baseDate = new Date()) {
    ...
    function getF1SeasonDates(year) {
    ...
    const seasons = [
        getF1SeasonDates(currentYear),
        getF1SeasonDates(lastYear)
    ];

    // Generate URLs for valid F1 season dates within the past year
    const startDate = new Date(today);
    // Loop through the past 365 days but only include dates in F1 seasons
    for (let i = 0; i < 365; i++) {
        startDate.setDate(startDate.getDate() - 1); // Move to previous day
        // Check if date falls within any of the defined F1 seasons
