...
function scoresURLs(baseDate = new Date()) {
    ...
    const urls = [];
    urls.push(`https://www.goalserve.com/getfeed/${API_KEY}/bsktbl/nba-scores?json=1`);

    // Get valid MLB season dates for the past year
    const currentYear = today.getFullYear();
    const lastYear = currentYear - 1;
    
    // Helper function to determine NBA season dates for a given year
    function getNBASeasonDates(year) {
    ...
    const seasons = [
        getNBASeasonDates(currentYear),
        getNBASeasonDates(lastYear)
    ];

    // Generate URLs for valid NBA season dates within the past year
    const startDate = new Date(today);
    // Loop through the past 365 days but only include dates in NBA seasons
    for (let i = 0; i < 365; i++) {
        startDate.setDate(startDate.getDate() - 1); // Move to previous day
        // Check if date falls within any of the defined NBA seasons
        ...
        if (isInSeason) {
            ...
            urls.push(`https://www.goalserve.com/getfeed/${API_KEY}/bsktbl/nba-scores?date=${dateString}&json=1`);
        }
    }
...
async function fetchNBAData(sport, extractedData) {
    const sportLower = sport.toLowerCase();
    if (sportLower !== 'nba') {
        throw new Error('Only NBA LS is valid for now');
    }

    const scoreUrls = scoresURLs();
    // const standingsUrl = `https://www.goalserve.com/getfeed/${API_KEY}/bsktbl/nba-standings?json=1`;
    let scheduleUrl = `https://www.goalserve.com/getfeed/${API_KEY}/bsktbl/nba-shedule?json=1`;
