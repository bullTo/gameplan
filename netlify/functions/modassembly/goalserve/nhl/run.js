...
function scoresURLs(baseDate = new Date()) {
    ...
    const urls = [];
    urls.push(`https://www.goalserve.com/getfeed/${API_KEY}/hockey/nhl-scores?json=1`);

    // Get valid MLB season dates for the past year
    const currentYear = today.getFullYear();
    const lastYear = currentYear - 1;
    
    // Helper function to determine NHLason dates for a given year
    function getNHLSeasonDates(year) {
    ...
    const seasons = [
        getNHLSeasonDates(currentYear),
        getNHLSeasonDates(lastYear)
    ];

    // Generate URLs for valid NHL season dates within the past year
    const startDate = new Date(today);
    // Loop through the past 365 days but only include dates in NHL seasons
    for (let i = 0; i < 365; i++) {
        startDate.setDate(startDate.getDate() - 1); // Move to previous day
        // Check if date falls within any of the defined NHL seasons
        ...
        if (isInSeason) {
            ...
            urls.push(`https://www.goalserve.com/getfeed/${API_KEY}/hockey/nhl-scores?date=${dateString}&json=1`);
        }
    }
...
async function fetchNHLData(sport, extractedData) {
    const sportLower = sport.toLowerCase();
    if (sportLower !== 'nhl') {
        throw new Error('Only NHL LS is valid for now');
    }

    const scoreUrls = scoresURLs();
    const standingsUrl = `https://www.goalserve.com/getfeed/${API_KEY}/hokey/nhl-standings?json=1`;
    let scheduleUrl = `https://www.goalserve.com/getfeed/${API_KEY}/hokey/nhl-shedule?json=1`;
